...
public class AddProfile extends PreferenceActivity implements
	...
	private CheckBoxPreference mProfActive, mProfSilent, mProfVibrate,
			mProfRing, mProfOverride, mProfCustom;
	private PreferenceScreen mEmailNotify, mSmsNotify, mMmsNotify,
	mPhoneNotify,mContactScreen;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mProfActive = (CheckBoxPreference) this
				.findPreference(KEY_PROFILE_ACTIVE);
		mProfSilent = (CheckBoxPreference) this
				.findPreference(KEY_PROFILE_SILENT);
		mProfVibrate = (CheckBoxPreference) this
				.findPreference(KEY_PROFILE_VIBRATE);
		mProfRing = (CheckBoxPreference) this.findPreference(KEY_PROFILE_RING);
		mProfOverride = (CheckBoxPreference) this
				.findPreference(KEY_PROFILE_OVERRIDE);
		mProfCustom = (CheckBoxPreference) this
				.findPreference(KEY_PROFILE_CUSTOM);
		 ...
		 */
		if (mProfName == null || mProfActive == null || mProfSilent == null
				|| mProfVibrate == null || mProfRing == null
				|| mProfOverride == null || mProfCustom == null || mContactScreen == null 
				|| mProfRingtone == null || mProfRingVolume == null) {
	...
	private void getNotification(Integer notifyType) {
		...
		if (notifyType == null) {
			Thread t = new Thread() {
				@Override
				public void run() {
					pPhone = Profiles.getNotifyByProfileId(
							getContentResolver(), (int) mProfileId,
							Notify.Columns.NOTIFY_TYPE_PHONE);
					pSms = Profiles.getNotifyByProfileId(getContentResolver(),
							(int) mProfileId, Notify.Columns.NOTIFY_TYPE_SMS);
					pMms = Profiles.getNotifyByProfileId(getContentResolver(),
							(int) mProfileId, Notify.Columns.NOTIFY_TYPE_MMS);
					pEmail = Profiles.getNotifyByProfileId(
							getContentResolver(), (int) mProfileId,
	...
	private void loadDefaultValues() {
		...
		if (mProfileId > 0) { // is id valid?
			...
			Profile profile = Profiles.getProfileByID(getContentResolver(),
					(int) mProfileId);
		...
		mProfile.put(Profile.Columns.OVERRIDES, mProfOverride.isChecked() ? 1
				: 0);
		mProfile.put(Profile.Columns.CUSTOM_NOTIFY, mProfCustom.isChecked() ? 1
				: 0);
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case MENU_SAVE:
			// TODO: validation needs to be done, make sure profile_name != null
			if (validate()) {
				saveProfile();
				// TODO: set this to show Saving or Updating depending on what
				// we are doing
				Toast.makeText(this, "Saving " + mProfName.getText(),
	...
	private void addContacts() {
		...
		// contacts
		// TODO:
		//-This is going to work the same as the overrides as it only stores _ID profile_id real_contact_id 
		// so delete old data and add new data
		// these methods run quite fast so I assume that should not be an issue on any android enabled device.
		...
		// fixme:  this is kind of lame, we need a query profile_id AND real_id = that would eliminate this
	...
	private void showCancelDialog() {
		new AlertDialog.Builder(this).setTitle("Dialog title").setIcon(
				android.R.drawable.ic_dialog_alert).setMessage(
				"Are you sure you want to cancel?").setPositiveButton("Ok",
	...
	public boolean onPreferenceClick(Preference pref) {
		...
		if (pref == mProfActive) {
			...
			mProfile.put(Profile.Columns.ACTIVE, mProfActive.isChecked() ? 1
					: 0);
		...
		if (pref == mProfSilent) {
			mProfile.put(Profile.Columns.SILENT, mProfSilent.isChecked() ? 1
					: 0);
		...
		if (pref == mProfVibrate) {
			mProfile.put(Profile.Columns.VIBRATE, mProfVibrate.isChecked() ? 1
					: 0);
		...
		if (pref == mProfOverride) {
			mProfile.put(Profile.Columns.OVERRIDES,
					mProfOverride.isChecked() ? 1 : 0);
			mProfOverride.setSummary(mProfOverride.isChecked() ? "yes" : "no");
			return true;
		}
		
		if (pref == mProfRingVolume) {
			mProfRingVolume.setVolume(Integer.valueOf(mProfile.get(Profile.Columns.RING_VOL).toString()));
		}

		if (pref == mProfCustom) {
			mProfile.put(Profile.Columns.CUSTOM_NOTIFY,
					mProfCustom.isChecked() ? 1 : 0);
		...
		if (pref == mEmailNotify) {
			...
			if (Log.LOGV) Log.v("email notify click start intent");
			
			Intent intent = new Intent(this, NotificationPreferences.class);
			intent.putExtra(Profiles.PROFILE_ID, mProfileId);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (Log.LOGV) Log.v(TAG + " onActivityResult requestCode " + requestCode + " resultCode " + resultCode);
